        ...
        if code:
            ...
            if use_llm:
                ...
                if llm_out.startswith("[LLM-missing]") or llm_out.startswith("[LLM-"):
                    st.warning("LLM unavailable or returned an error. Cannot handle this custom prompt.")
                    st.write(llm_out)
                    st.stop()
                ...
                st.error("This is a custom prompt the app cannot convert. Enable 'Use local LLM' or change prompt.")
                st.stop()
